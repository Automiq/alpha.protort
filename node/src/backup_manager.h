   ...
   void backup_is_life(const alpha::protort::protocol::backup::Packet& packet){
    ...
    void backup_transition(){
        ...
        if(node_status_==Node_status::slave){
            switch_status();
        //    master_manager_.get_deleter();//спросить
        //    return true;
        }
        else{
            alpha::protort::protocol::Packet_Payload packet;
    ...
    void start_keepalife(){
        if(node_status_==Node_status::slave){
            master_manager_=std::make_unique<master_manager>(service__ , 500 , client_);
            master_manager_->get_signal()->on_timeout.connect([&](){
                backup_transition();
            });
            master_manager_.get()->start_check();
            //    backup_transition();
        }
    }
...
private:
    ...
    std::unique_ptr<alpha::protort::node::master_manager> master_manager_;
    //
    client_ptr client_;
};
