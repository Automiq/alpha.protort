...
void RemoteNode::init(boost::asio::io_service &service)
{
    ...
    boost::asio::ip::tcp::endpoint ep(
                boost::asio::ip::address::from_string(node_information_.address_.ip_address),
                node_information_.address_.config_port);
...
{
    return QString::fromStdString(node_information_.address_.ip_address);
}
...
void RemoteNode::async_deploy(deploy_configuration& deploy_configuration)
{
    ...
    configuration->mutable_this_node_info()->set_port(deploy_configuration.map_node[current_node].address_.port);

    alpha::protort::protocol::deploy::NodeInfo* node_info_ = configuration->add_node_infos();
    node_info_->set_name(current_node);
    node_info_->set_port(deploy_configuration.map_node[current_node].address_.port);
    node_info_->set_address(deploy_configuration.map_node[current_node].address_.ip_address);
        ...
        for (auto &connection : deploy_configuration.map_component_with_connections[component.comp_name])
        {
            ...
            if(node_name != current_node)
            {
                ...
                if(added_nodes.find(node_.name) == added_nodes.end())
                {
                    ...
                    remote_node_info_->set_port(node_.address_.port);
                    remote_node_info_->set_address(node_.address_.ip_address);
