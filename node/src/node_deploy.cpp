...
void node_deploy::load_config(alpha::protort::parser::configuration &conf)
{
...
void node_deploy::deploy(node &_node)
{
    current_node_name = _node.settings_.name;
    ...
    for (auto & name_to_comp : _node.router_.components){
        ...
        for (const auto & port_to_conn : comp_info.connections){
            const port_id& out_port = port_to_conn.first;

            // Для каждого получателя
            for(const auto & dest : port_to_conn.second){
                const std::string& dest_node_name = components_[dest.comp_name].node_name;
